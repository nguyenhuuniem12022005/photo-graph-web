...
function UserPhotos(props) {
  ...
  const { advancedFeaturesEnabled } = props;
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (advancedFeaturesEnabled && photoData.length > 0) {
            const hasMatchingPhotoId = photoData.some((photo) => photo._id === photoId);
  ...
  const renderPhotoCard = (photo) => (
    <Card key={photo._id} variant="outlined">
      <img
        ...
        src={getPhotoPath(photo.file_name)}
  ...
  if (error) {
    return <Alert severity="error">Unable to load photos: {error}</Alert>;
  }
